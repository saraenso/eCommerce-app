...
export function createStorage() {
  ...
  async function removeAllOrders() {
    try {
      ...
      if (orders && orders.length > 0) {
        ...
        window.location.href = "order.html"; // переадресация происходит только после успешной очистки всех заказов
      } else {
  ...
  async function clearOrders(orders) {
    try {
    ...
    try {
        ...
        if (!querySnapshot.empty) {
            ...
            if (newQuantity <= 0) {
                ...
                console.log(`Deleted order with ID ${itemID} because quantity was ${newQuantity}`);
            } else {
                await updateDoc(docRef, { quantity: newQuantity });
                console.log(`Updated quantity of item with ID ${itemID} to ${newQuantity}`);
            }
        } else {
